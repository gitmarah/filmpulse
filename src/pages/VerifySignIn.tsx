  ...
  return (
    ...
    <main className="w-full h-[100vh] flex flex-col justify-center items-center gap-1 bg-[#f0ffff]">
      ...
      <form onSubmit={signInFunction} className="flex flex-col items-center gap-2 min-w-70">
        ...
        <div className="w-full relative">
            ...
            <div className="w-full relative">
                <input
                    ...
                    className="peer w-full border border-[#ddd] focus:border-[#da0009] rounded-sm text-[0.9rem] outline-none px-3 py-2 bg-transparent"
                    onChange={(e:React.ChangeEvent<HTMLInputElement>) => handleFormState(e)}
                />
                <label
                    htmlFor="password"
        ...
        <button type="submit" className="bg-[#da0009] w-full py-2 text-[1rem] font-black text-[#fff1fc] rounded-sm mt-[0.15rem] cursor-pointer transition-all hover:opacity-85 flex items-center justify-center">{isLoading ? <Loader /> : "SignIn"}</button>
      </form>
