...
const MovieDetail = () => {
    ...
    const {id} = useParams()
    const {data, isLoading} = useGetMovieQuery(id ?? skipToken)
    ...
    return (
        <>
            <Header />
            <main className={`bg-[#f0ffff] max-w-[100vw] min-h-[100vh] flex flex-col items-center pb-4 pt-[4rem] ${(data && !data.backdrop_path) && 'pt-[11rem]'}`}>
                {isLoading ? <p className='text-center mt-20'>Loading...</p> : <div className='max-w-2xl'>
                    ...
                    <div className='pl-32 pr-3 text-[#170000] w-full'>
                        <h1 className='font-bold text-ellipsis overflow-hidden whitespace-nowrap'>{data?.title}</h1>
                        <p className='text-sm -mt-[3px]'>{release_date}</p>
                        <p className='text-sm -mt-[3px]'>{data?.status}</p>
                    </div>
                    <div className='p-3'>
                        <div className='flex flex-wrap gap-2 justify-center bg-amber-300 py-1 rounded-sm mb-2'>
                        ...
                        {data?.budget ? <><h2 className='font-bold text-[#da0009]'>Budget</h2>
                        <p className='text-sm text-[#170000] mb-2'>
                            {data?.budget.toLocaleString("en-US", { style: "currency", currency: "USD", })}
                        </p></> : ""}
                        {data?.revenue ? <><h2 className='font-bold text-[#da0009]'>Revenue</h2>
                        <p className='text-sm text-[#170000] mb-2'>
